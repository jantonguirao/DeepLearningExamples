...
def _generate_cutouts(mask_params, nfeatures):
    ...
    shapes.extend(
        _interleave_lists(
            np.random.randint(mask_params['freq_min'], mask_params['freq_max'] + 1,
    ...
    shapes.extend(
        _interleave_lists(
            ...
            np.random.randint(mask_params['time_min'], mask_params['time_max'] + 1,
                                mask_params['time_num_regions'])
        )
    )
    shapes.extend(
        _interleave_lists(
            np.random.randint(mask_params['both_min_freq'], mask_params['both_max_freq'] + 1,
                                mask_params['both_num_regions']),
            np.random.randint(mask_params['both_min_time'], mask_params['both_max_time'] + 1,
                                mask_params['both_num_regions'])
        )
    )
    return anchors, shapes

def _cutouts_generator(mask_params, nfeatures, batch_size):
    """
    ...
    [anchors, shapes] = tuples2list(
        [_generate_cutouts(mask_params, nfeatures) for _ in range(batch_size)]
    )
...
@dali.pipeline_def
def dali_jasper_pipe(train_pipeline: bool,  # True if train pipeline, False if validation pipeline
                     file_root: str,
                     file_list: str,
                     config_data: dict,
                     ...
                     shard_id: int = 0,
                     n_shards: int = 1,
                     preprocessing_device: str = "gpu",
                     do_resampling: bool = True,
                     is_triton_pipeline: bool = True):
    ...
    mel_spec = fn.mel_filter_bank(spec, sample_rate=sample_rate, nfilter=nfeatures, normalize=True)
    ...
    if train_pipeline and do_spectrogram_masking:
        anchors, shapes  = fn.external_source(
            source=lambda: _cutouts_generator(mask_params, nfeatures),
...
class DaliPipelineFactory():
    ...
    def from_config(cls, train_pipeline: bool, device_id, batch_size, file_root: str, file_list: str, config_data: dict,
                    ...
                    is_triton_pipeline: bool = False, num_cpu_threads: int = multiprocessing.cpu_count()):
        if torch.distributed.is_initialized():
        ...
        pipe = dali_jasper_pipe(
            train_pipeline=train_pipeline, file_root=file_root, file_list=file_list,
            config_data=config_data, config_features=config_features,
            shard_id=shard_id, n_shards=n_shards,
            preprocessing_device=preprocessing_device, do_resampling=do_resampling,
...
def serialize_dali_triton_pipeline(output_path: str, config_data: dict, config_features: dict):
    pipe = DaliPipelineFactory.from_config(train_pipeline=False, is_triton_pipeline=True,
                                           device_id=-1, batch_size=-1, num_cpu_threads=-1,
                                           file_root=None, file_list=None, do_resampling=False,
                                           config_data=config_data, config_features=config_features)
